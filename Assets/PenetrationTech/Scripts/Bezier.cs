...
namespace PenetrationTech {
    public static class Bezier {
        ...
        public static Vector3 BezierSlope(Vector3 startPosition, Vector3 startTangent, Vector3 endTangent, Vector3 endPosition, float t) {
            float s = 1.0f - t;
            return s * s * 3 * (startTangent - startPosition) + 6 * s * t * (endTangent - startTangent) + 3 * t * t * (endPosition - endTangent);
        }
        ...
        static Vector3 ClosestPointOnCurveIterative(Vector3 point, Vector3 startPosition, Vector3 endPosition, Vector3 startTangent, Vector3 endTangent, float sqrError, ref float startT, ref float endT) {
        ...
        public static float BezierApproxLength(Vector3 startPosition, Vector3 endPosition, Vector3 startRightTangent, Vector3 endLeftTangent) {
            float chord = Vector3.Distance(endPosition, startPosition);
            float cont_net = Vector3.Distance(startPosition,startRightTangent) + Vector3.Distance(startRightTangent,endLeftTangent) + Vector3.Distance(endPosition,endLeftTangent);
